  ...
  return (
    <div className="invoice-document bg-white p-6 sm:p-10 rounded-lg border border-ink-300/60 shadow-card max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
        <div>
          <p className="text-xs uppercase tracking-wide text-ink-500 mb-1">Bill To</p>
          <p className="text-sm font-semibold text-ink-900">{invoice.billTo?.contactName || "—"}</p>
          {invoice.billTo?.companyName && <p className="text-sm text-ink-700">{invoice.billTo.companyName}</p>}
          {invoice.billTo?.address && <p className="text-sm text-ink-700 whitespace-pre-line">{invoice.billTo.address}</p>}
          {invoice.billTo?.phone && <p className="text-sm text-ink-700">{invoice.billTo.phone}</p>}
          {invoice.billTo?.email && <p className="text-sm text-ink-700">{invoice.billTo.email}</p>}
        </div>
        <div>
          <p className="text-xs uppercase tracking-wide text-ink-500 mb-1">Ship To</p>
          <p className="text-sm font-semibold text-ink-900">{invoice.shipTo?.name || "—"}</p>
          {invoice.shipTo?.companyName && <p className="text-sm text-ink-700">{invoice.shipTo.companyName}</p>}
          {invoice.shipTo?.address && <p className="text-sm text-ink-700 whitespace-pre-line">{invoice.shipTo.address}</p>}
          {invoice.shipTo?.phone && <p className="text-sm text-ink-700">{invoice.shipTo.phone}</p>}
        </div>
      </div>
          ...
          <tbody>
            {items.map((it, i) => (
              <tr key={i} className="border-b border-ink-300/50">
                <td className="py-2 text-ink-900">{it.description || it.product?.name || "—"}</td>
